...
# --- UI COMPONENTS (CSS Fix Applied) ---
def get_layout(content):
    # CSS ব্র্যাকেটগুলো এখানে ডাবল {{ }} করা হয়েছে যাতে পাইথন এরর না দেয়
    header = f"""
    <meta charset="UTF-8">
...
def home():
    ...
    """
    return render_template_string(get_layout(content), keys=db.keys(), db=db)
...
def admin():
    ...
    """
    return render_template_string(get_layout(content), keys=db.keys())
