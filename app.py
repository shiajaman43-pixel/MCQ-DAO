...
# --- CONFIGURATION ---
# Apnar deya notun Smartlink
SMARTLINK_URL = "https://potterynaggingformerly.com/surggewa?key=91990ae75a2cedbea643e7b2b13aadf6" 
...
def main_tab():
    ...
    <div class="container mt-4 text-center">
        <h3 class="fw-bold mb-4">Exam Preparation Portal</h3>
        {NATIVE_AD}

        <div style="margin-top: 40px;">
            <button onclick="startPrep()" class="btn-main shadow-lg">🚀 START PREPARATION</button>
            <p class="text-muted mt-3 small">If it doesn't load, click again</p>
        </div>
        ...
        <div id="loading" class="hidden" style="position:fixed; top:0; left:0; width:100%; height:100%; background:white; z-index:10000; text-align:center; padding-top:100px;">
            <h3>Redirecting to Exam Server...</h3>
    ...
    <script>
    function startPrep() {{
        // Auto-open Smartlink on button click
...
def exam():
    ...
    q_html = "".join([f'<div class="card p-3"><b>{q}</b><br><input type="radio" name="q{i}"> Answer A &nbsp; <input type="radio" name="q{i}"> Answer B</div>' for i, q in enumerate(questions)])

    return render_template_string(UI_STYLE + f"""
    <div class="container mt-3">
        <div id="quiz-ui">
            <h4 class="text-center mb-4">ICT Challenge (10 Qs)</h4>
            {q_html}
            <button onclick="submitExam()" class="btn btn-success w-100 p-3 mb-5 fw-bold shadow">SUBMIT FOR RESULT</button>
        </div>
    ...
    <script>
    function submitExam() {{
        // Smartlink trigger again on submit
        window.open('{SMARTLINK_URL}', '_blank');
...
def result():
    ...
    <div class="container mt-5 text-center">
        <div class="card p-5 shadow">
            ...
            <p class="lead">Excellent! You've mastered ICT.</p>
            <hr>
            {NATIVE_AD}
            <a href="/" class="btn btn-dark w-100 p-3 mt-3 shadow">Restart Preparation</a>
        </div>
    </div>
    """)
